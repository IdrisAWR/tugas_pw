        ...
        <form action="{{ route('pembayaran.store') }}" method="POST" class="space-y-4">
            ...
            <div class="p-4 border rounded-lg bg-gray-50">
                ...
                <div class="flex justify-between">
                    ...
                    <input type="hidden" name="pembayarans[{{ $index }}][tgl_pembayaran]" value="{{ date('Y-m-d') }}">
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Periode Pembayaran:</span>
                    <input type="text" name="pembayarans[{{ $index }}][periode_pembayaran]" class="bg-white border rounded-md p-2 w-full text-gray-700" value="{{ $pembayaran->periode_pembayaran }}">
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Jumlah:</span>
                    <input type="number" name="pembayarans[{{ $index }}][jumlah]" class="bg-white border rounded-md p-2 w-full text-gray-700" value="{{ $pembayaran->jumlah }}">
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-500">Status:</span>
                    <select name="pembayarans[{{ $index }}][status]" class="bg-white border rounded-md p-2 text-gray-700">
                ...
                <div class="flex justify-between">
                    ...
                    <input type="hidden" name="pembayarans[{{ $index }}][user_id]" value="{{ $pembayaran->user->id }}">
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Nama Paket:</span>
                    <span class="font-medium">{{ $pembayaran->paket->nama_paket }}</span>
                    <input type="hidden" name="pembayarans[{{ $index }}][paket_id]" value="{{ $pembayaran->paket->id }}">
                </div>
            </div>
            @endforeach
